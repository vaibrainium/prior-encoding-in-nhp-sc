...
def session_wise_fit_cv(observations, inputs, masks, n_sessions, init_params, k_folds=5, state_range=np.arange(2, 6), fitting_method="em", n_iters=200, tolerance=10**-4, n_jobs=-1):
	...
	def fit_model_on_fold(train_idx, test_idx, glm_hmm, idx_session):
		...
		"""
		train_ll = glm_hmm.fit(observations[idx_session][train_idx], inputs=inputs[idx_session][train_idx], masks=masks[idx_session][train_idx], method=fitting_method, num_iters=n_iters, initialize=False, tolerance=tolerance)
		test_ll = glm_hmm.log_likelihood(observations[idx_session][test_idx], inputs=inputs[idx_session][test_idx], masks=masks[idx_session][test_idx])
	...
	def process_session_state_fold(idx_session, n_states):
		...
		results = Parallel(n_jobs=n_jobs)(delayed(fit_model_on_fold)(train_idx, test_idx, glm_hmm, idx_session) for train_idx, test_idx in kf.split(np.arange(observations[idx_session].shape[0])))
